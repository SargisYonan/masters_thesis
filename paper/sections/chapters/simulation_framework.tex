...
\begin{figure}[htb!]
	...
	\captionsetup{skip=0.25\baselineskip}
	\ssp
    \caption{Monte Carlo path planner run ($N=5$, $M_{mc}=20$). The actual field (top left), predicted field (with error) (top right), variance field (bottom left), and traversed path (bottom right). The path planner is conducted on the same field with an approximate 3\% scan area limit.}
\end{figure}
\begin{figure}[htb!]
    \centering
